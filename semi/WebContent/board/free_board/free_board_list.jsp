...
<script type="text/javascript">
	...
	<div align="center" id="main">
		...
		<div id="freeBoard_order_list">
			<select name="field" onchange="onOrderChange(this.value)" style="width:10%">
			...
			success : function(response) {
				...
				if(response.list.length > 0) {
					...
					$.each(response.list, function(index, board) {
						...
						html += "</td>"
							+ "<td style='text-align: left;'><a href='<%=request.getContextPath() %>/board_content.do?no=" + board.board_no + "&page=" + response.page + "&id=${user_id }&nickname=" + board.user_nickname + "&orderBy=time&board_name="+ board.board_name +"'>" +"["+board.board_category+"]"+"&nbsp;"+ board.board_title + "</a></td>"
									+ "<td>"+board.user_nickname+"</td>"
									+ "<td>"+board.board_date.substring(0,10)+"</td>"
									+ "<td>"+board.board_hit+"</td>"
									+ "</tr>";
		    ...
		    success: function(response) {
		      ...
		      if (response.list.length > 0) {
		        ...
		        $.each(response.list, function(index, board) {
		          ...
		          html += "</td>" +
		            "<td style='text-align: left;'><a href='<%=request.getContextPath() %>/board_content.do?no=" + board.board_no + "&page=" + response.page + "&id=${user_id }&nickname=" + board.user_nickname +"&orderBy=look&board_name="+ board.board_name + "'>" +"["+board.board_category+"]"+"&nbsp;"+ board.board_title + "</a></td>" +
		            "<td>" + board.user_nickname + "</td>" +
		            "<td>" + board.board_date.substring(0, 10) + "</td>" +
		            "<td>" + board.board_hit + "</td>" +
		            "</tr>";
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("select[name='field'] option[value='" + selectedOrder + "']").attr('selected', true);
   		...
   		<div id="Board_table">
	   		<%-- 자유게시판 테이블 --%>
			<table border="1" cellspacing="0" class="board_table_inner">
