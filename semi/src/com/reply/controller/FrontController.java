...
public class FrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * java.util.Propertise 클래스
		 * - Properties 클래스는 HashTable의 하위(자식) 클래스.
		 * - 보통은 환경변수 및 속성 값을 Properties 파일에 저장하여 쉽게 접근할 수 있는 장점이 있음.
		 * - Properties 파일을 일련의 키(key) - 값(value)의 한 쌍으로 이루어져 있음.
		 * - 보통을 파일에 저장을 하여 사용을 함. 파일 이름을 *.properties 라는 이름으로 끝나게 함.
		 * - FileInputStream 클래스에 Properties 파일을 인자로 넣어서 그 스트림으로부터 파일을 가져올 때
		 *   많이 사용을 함. 인자로 들어온 Properties 파일을 읽게 됨.
		 * - 읽어 들일 때 사용하는 메서드는 load() 라는 메서드를 이용하여 파일을 읽어 들이게 됨.
		 */
		
		String path2 = FrontController.class.getResource("").getPath();
	      
	    FileInputStream fis = new FileInputStream("C:\\Users\\YBG\\Documents\\GitHub\\new_semi_project\\semi\\src\\com\\reply\\controller\\mapping.properties");
		...
		}else { // value 에 들어온 값이 "execute" 가 아닌 경우
			// view page 로 이동
