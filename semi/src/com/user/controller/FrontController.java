...
import com.user.action.ActionForward;
...
public class FrontController extends HttpServlet{
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	 ...
    	 *   - 보통은 환경변수 및 속성 값을 Properties 파일에 저장하여 쉽게 접근할 수 있는 장점.
    	 *   - Properties 파일은 일련의 키(key) - 값(value)의 한 쌍으로 이루어져 있음.
    	 *   - 보통은 파일에 저장을 하여 사용. 파일 이름을 *.properties 라는 이름으로 끝나게 함.
    	 *   - FileInoutStream 클래스에 Properties 파일을 인자로 넣어서 그 스트림으로부터 파일을 가져올 때 많이 사용
    	 *    인자로 들어온 Properties 파일을 읽게 됨.
    	 *   - 읽어 들일 때 사용하는 메서드는 load() 라는 메서드를 이용하여 파일을 읽어 들이게 됨.
    	 *   
    	 */
    	
    	FileInputStream fis = new FileInputStream("/Users/jeongbinkim/Desktop/JAVA/NCS/WORKSPACE(JSP)/00_login/src/main/java/com/user/controller/mapping.properties");
    ...
    }else { // value에 들어온 값이 execute가 아닌경우 view page 로 이
